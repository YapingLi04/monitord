...
mod tests {
    ...
    fn test_active_state_display() {
        ...
        assert_eq!(format!("{}", SystemdUnitActiveState::reloading), "reloading");
        assert_eq!(format!("{}", SystemdUnitActiveState::inactive), "inactive");
        assert_eq!(format!("{}", SystemdUnitActiveState::failed), "failed");
        assert_eq!(format!("{}", SystemdUnitActiveState::activating), "activating");
        assert_eq!(format!("{}", SystemdUnitActiveState::deactivating), "deactivating");
    ...
    fn test_active_state_equality() {
        assert_eq!(SystemdUnitActiveState::active, SystemdUnitActiveState::active);
        assert_ne!(SystemdUnitActiveState::active, SystemdUnitActiveState::inactive);
    }
    ...
    fn test_active_state_from_int() {
        assert_eq!(SystemdUnitActiveState::try_from(0).unwrap(), SystemdUnitActiveState::unknown);
        assert_eq!(SystemdUnitActiveState::try_from(1).unwrap(), SystemdUnitActiveState::active);
        assert_eq!(SystemdUnitActiveState::try_from(2).unwrap(), SystemdUnitActiveState::reloading);
        assert_eq!(SystemdUnitActiveState::try_from(3).unwrap(), SystemdUnitActiveState::inactive);
        assert_eq!(SystemdUnitActiveState::try_from(4).unwrap(), SystemdUnitActiveState::failed);
        assert_eq!(SystemdUnitActiveState::try_from(5).unwrap(), SystemdUnitActiveState::activating);
        assert_eq!(SystemdUnitActiveState::try_from(6).unwrap(), SystemdUnitActiveState::deactivating);
    }

    #[test]
    fn test_load_state_from_int() {
        assert_eq!(SystemdUnitLoadState::try_from(0).unwrap(), SystemdUnitLoadState::unknown);
        assert_eq!(SystemdUnitLoadState::try_from(1).unwrap(), SystemdUnitLoadState::loaded);
        assert_eq!(SystemdUnitLoadState::try_from(2).unwrap(), SystemdUnitLoadState::error);
        assert_eq!(SystemdUnitLoadState::try_from(3).unwrap(), SystemdUnitLoadState::masked);
        assert_eq!(SystemdUnitLoadState::try_from(4).unwrap(), SystemdUnitLoadState::not_found);
    }
    ...
    fn test_is_unit_unhealthy_all_combinations() {
        ...
        assert!(is_unit_unhealthy(SystemdUnitActiveState::reloading, SystemdUnitLoadState::loaded));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::inactive, SystemdUnitLoadState::loaded));
        ...
        assert!(is_unit_unhealthy(SystemdUnitActiveState::deactivating, SystemdUnitLoadState::loaded));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::unknown, SystemdUnitLoadState::loaded));

        // Test masked state with all active states (always healthy)
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::active, SystemdUnitLoadState::masked));
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::reloading, SystemdUnitLoadState::masked));
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::inactive, SystemdUnitLoadState::masked));
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::failed, SystemdUnitLoadState::masked));
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::activating, SystemdUnitLoadState::masked));
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::deactivating, SystemdUnitLoadState::masked));
        assert!(!is_unit_unhealthy(SystemdUnitActiveState::unknown, SystemdUnitLoadState::masked));
        ...
        assert!(is_unit_unhealthy(SystemdUnitActiveState::reloading, SystemdUnitLoadState::error));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::inactive, SystemdUnitLoadState::error));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::failed, SystemdUnitLoadState::error));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::activating, SystemdUnitLoadState::error));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::deactivating, SystemdUnitLoadState::error));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::unknown, SystemdUnitLoadState::error));

        // Test not_found state with all active states (always unhealthy)
        assert!(is_unit_unhealthy(SystemdUnitActiveState::active, SystemdUnitLoadState::not_found));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::reloading, SystemdUnitLoadState::not_found));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::inactive, SystemdUnitLoadState::not_found));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::failed, SystemdUnitLoadState::not_found));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::activating, SystemdUnitLoadState::not_found));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::deactivating, SystemdUnitLoadState::not_found));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::unknown, SystemdUnitLoadState::not_found));

        // Test unknown state with all active states (always unhealthy)
        assert!(is_unit_unhealthy(SystemdUnitActiveState::active, SystemdUnitLoadState::unknown));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::reloading, SystemdUnitLoadState::unknown));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::inactive, SystemdUnitLoadState::unknown));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::failed, SystemdUnitLoadState::unknown));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::activating, SystemdUnitLoadState::unknown));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::deactivating, SystemdUnitLoadState::unknown));
        assert!(is_unit_unhealthy(SystemdUnitActiveState::unknown, SystemdUnitLoadState::unknown));
    }
