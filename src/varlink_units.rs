...
const METRICS_SOCKET_PATH: &str = "/run/systemd/report/io.systemd.Manager";
...
pub fn parse_one_metric(stats: &mut SystemdUnitStats, metric: &ListOutput) -> Result<()> {
...
pub async fn parse_metrics(stats: &mut SystemdUnitStats) -> Result<(), anyhow::Error> {
    let metrics: Vec<ListOutput> = tokio::task::spawn_blocking(|| {
...
mod tests {
    ...
    async fn test_update_unit_stats() {
        ...
        let result = update_unit_stats(Arc::new(config), machine_stats.clone()).await;
